    ...
    useEffect(() => {
        ...
        const updatePlayhead = () => {
            ...
            if (isPlaying && videoElement) {
                // When playing, ALWAYS use the direct video element time for smooth sync
                // React state (externalCurrentTime) is too slow/batched causing lag ('following late')
                time = videoElement.currentTime;

                // Gap Muting Logic: Check if we are in a valid audio section
                // 1. Check video clips (that have audio)
                const hasVideoAudio = videoClips.some(clip =>
                    clip.hasAudio !== false && time >= clip.startTime && time < clip.endTime
                );
                // 2. Check unlinked audio clips
                const hasAudioClip = localAudioClips.some(clip =>
                    time >= clip.startTime && time < clip.endTime
                );

                // If in a gap (no audio), mute the video element
                const shouldPlayAudio = hasVideoAudio || hasAudioClip;
                if (videoElement.muted === shouldPlayAudio) {
                    // Only toggle if state is different (avoid unnecessary writes)
                    videoElement.muted = !shouldPlayAudio;
                }
            } else {
            ...
            if (playheadRef.current && (isPlaying || externalCurrentTime !== undefined) && !isScrubbing) {
                ...
                if (containerRef.current) {
                    ...
                    // Ideally, we want the playhead to be at the center
                    const targetScrollLeft = currentPx - halfWidth;

                    if (isPlaying) {
                        // During playback, if playhead drifts too far from center (e.g. 100px), smooth scroll to it
                        const currentScrollLeft = container.scrollLeft;
                        const diff = targetScrollLeft - currentScrollLeft;

                        // If difference is significant, scroll to keep it centered
                        // Using a small lerp factor for smoothness
                        if (Math.abs(diff) > 50) {
                            container.scrollLeft = currentScrollLeft + diff * 0.1;
                        }
                    }
                    // REMOVED: Force snap when paused. This prevented manual scrolling.
                }
            }
