...
# --- [ADD] uniqueId만 오면 내부 루프백으로 secUid 自动查询/auto-resolve ---
def _ensure_secuid(uniqueId: str | None, secUid: str | None) -> tuple[str | None, str | None]:
    if secUid or not uniqueId:
        return uniqueId, secUid
    try:
        import requests
        r = requests.get(
            "http://127.0.0.1:8000/api/tiktok/web/get_sec_user_id",
            params={"url": f"https://www.tiktok.com/@{uniqueId}"},
            timeout=10,
        )
        if r.ok:
            j = r.json()
            # 빌드마다 data가 문자열(secUid) 또는 객체일 수 있음
            suid = j.get("data")
            if isinstance(suid, dict):
                suid = suid.get("secUid")
            if isinstance(suid, str) and suid:
                secUid = suid
    except Exception:
        pass
    return uniqueId, secUid
...
async def fetch_user_profile(request: Request,
    ...
    # --- [ADD] secUid 자동 보강 ---
    uniqueId, secUid = _ensure_secuid(uniqueId, secUid)
